...
export interface UseOrganizationManagerReturn {
    ...
    createOrganization: ReturnType<typeof useOrganizationList>['createOrganization'];
}
...
export async function useOrganizationManager(): Promise<UseOrganizationManagerReturn> {
    ...
    const isAdmin = await organization?.getRoles().then((roles) => roles.data.some((role) => role.name === 'org:admin'));
    const isMember = await organization?.getRoles().then((roles) => roles.data.some((role) => role.name === 'org:member'));
