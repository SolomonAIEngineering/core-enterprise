...
const meta: Meta<typeof FilterSelect> = {
    ...
    parameters: {
        ...
        docs: {
            description: {
                component: 'A dropdown component for selecting and managing filters with search, keyboard shortcuts, and AI capabilities.'
            },
        },
    },
    tags: ['autodocs'],
...
export const Default: Story = {
    render: () => {
        const [activeFilters, setActiveFilters] = useState<Array<{ key: string; value: FilterOption['value'] }>>([]);
        ...
        const handleSelect = (key: string, value: FilterOption['value']) => {
            ...
            if (filter?.multiple) {
                ...
                setActiveFilters([...activeFilters.filter(f => f.key !== key), { key, value }]);
            }
        };
        ...
        return (
            <div className="w-[300px]">
                <FilterSelect
                    filters={filters}
                    activeFilters={activeFilters}
                    onSelect={(key, value) => setActiveFilters([...activeFilters, { key, value }])}
                    onRemove={(key, value) => {
                        setActiveFilters(activeFilters.filter(f => !(f.key === key && f.value === value)));
                    }}
                    emptyState={{
