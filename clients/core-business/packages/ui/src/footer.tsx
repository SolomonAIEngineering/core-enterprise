...
export function Footer({
  ...
  let { domain = 'getvector.io' } = useParams() as { domain: string }
  if (staticDomain) {
    domain = staticDomain
  }
...
function StatusBadge() {
  ...
  }>(`${platform.statusPageUrl}.com/api/v1/summary`, fetcher)
  ...
  return (
    <Link
      href={`${platform.statusPageUrl}`}
      target="_blank"
      className="group flex max-w-fit items-center gap-2 rounded-lg border border-neutral-200 bg-white py-2 pl-2 pr-2.5 transition-colors hover:bg-neutral-50 active:bg-neutral-50"
    >
