...
export default function UpdateHistoryRetention({
  ...
  const [selectedOption, setSelectedOption] = useState<ComboboxOption | null>(
    HISTORY_RETENTION_OPTIONS.find(
      (opt) => opt.value === String(currentRetentionDays),
    ) || null,
  );
  ...
  const handleUpdateRetention = async () => {
    if (
      !selectedOption ||
      Number(selectedOption.value) === currentRetentionDays
    )
      return;
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Error updating history retention",
      );
    } finally {
      setSaving(false);
    }
    ...
    >
      <div className="flex flex-col space-y-3 p-5 sm:p-10">
        ...
        <div className="flex items-center justify-between space-x-4 rounded-b-lg border border-gray-200 bg-gray-50 p-3 sm:px-10">
          <p className="text-sm text-gray-500">
            Older conversations will be automatically deleted after the selected
            period.
          </p>
          <div>
            <Button
