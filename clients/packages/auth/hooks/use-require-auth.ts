...
export function useRequireAuth({
    ...
    useEffect(() => {
        ...
        if (requireVerification && user?.primaryEmailAddress?.verification.status !== 'verified') {
            router.push('/verify-email');
        }
    }, [isLoaded, isSignedIn, redirectTo, requireVerification, router, user?.primaryEmailAddress?.verification.status]);
